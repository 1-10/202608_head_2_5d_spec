...
//
// **シーンは書き出しの直後に作る。** デスクトップ側は 3D ビューを別ウィンドウで開くので
// `build_viewer_scene(enabled, source)` が「開いていなければ作らない」門を持つが、ブラウザでは
// 3D ビューが画面の主役で常に見えているため、門を置く先が無い。作らない選択肢が無いので
// `DebugSceneSource` の遅延も持たない（参照を束ねる型は `application` 側に残してある）。
//
