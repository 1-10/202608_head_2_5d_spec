...
// numpy のチャンク定数は持たない
// ---------------------------------
// デスクトップ側は `CANDIDATE_CHUNK_BUDGET` / `INTERPOLATION_CHUNK` で「同時に持つ候補の数」を
// 区切っている。あれは**ベクトル化のために中間配列を作る**（2048² で 1000 万件・数百 MB）ことの
// 対価で、こちらはテクセルごとの素直なループなので中間配列そのものが無い。同じ理由で
// `domain/photo` の `RESAMPLE_ROW_BUDGET` も持たない。
//
// アトラス配列の座標規約（ここが正本）
// ------------------------------------
