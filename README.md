...
トラッキング」「録画 / 保存 / 読み込み / 再生（ループ）」で、今どの状態か（停止中 / トラッキング中 /
録画中 / 再生中）をパネルの頭に出す。映像は**ミラー表示**だが、取り込んだ写真は非反転のまま扱う
（表示だけを CSS で反転している）。
...
| ウィンクの閾値 | 左右差 0.25 超で `wink_left` / `wink_right` へ | 同 `WINK_ASYMMETRY_THRESHOLD` |
| 録画の上限 | 30 秒 | `domain/preview/recording` の `MAX_RECORDING_SECONDS` |

**首・顔の向きは連動させない。** 写すのは表情だけ。頭部姿勢はスライダーとマウス追従が持っており、
そこへカメラの姿勢を足すと「今どちらが向きを決めているか」が画面から読めなくなる。
