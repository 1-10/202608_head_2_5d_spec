...
| 注視点（頭部中心）へ向き直す・初期値に戻す | 同じ節のボタン |
| 層の表示・層ごとのテクスチャ・ワイヤーフレーム | 右パネルのチェック |
| 検査画像・内訳を閉じる | `Esc` |

**キーの割り当ては持たない**（`Esc` だけ）。層・テクスチャ・ワイヤー・視点のリセットは単独キー
（`1`-`4` / `A` `S` `D` `F` / `T` / `W` / `R`）でも触れたが、同じことを右パネルのチェックとボタンでも
触れる。**同じ操作の入口を 2 つ持つと、片方だけ状態が動く経路を塞ぎ続けることになる**（実際に
`syncViewControls` がそのために在った）。入口はパネルひとつにした。
...
**状態は 3D ビューの中央に出す。置き場はここだけ。** 処理中は経過秒を**毎フレーム書き直す**
（推論は 30 秒かかることがあり、段の切り替わりでしか書かないと進んでいるのか固まったのか
分からない）。終わった表示は残るので **× で閉じられる**。進行（段の名前と何段目か・済んだ段・今の段・
...
**ただし目の予算は口と分ける。** 上限を全プリセットで 1 つにすると、喋りながら片目を閉じたときに
ウィンクが道連れで縮む（口が開くと `stretch_face` などで予算が埋まり、比例縮小がウィンクにも掛かる）
— **「片目を閉じても閉じない」と見えていた原因がこれ**。分ける根拠は、加算変位が壊すのは同じ領域を
動かし合ったときで、目と口は別の領域だから。

**カメラの首はリグの可動域へ写す。** リグは首 ±15° / pitch ±12° しか動かないので、生の角度をそのまま
渡すと少し振っただけで上限に張り付き、壁に当たった感じになる。人が振る幅（yaw ±35° / pitch ±25°）を
可動域へ線形に写す。**pitch は符号を返す** — 行列から出る pitch とビューの `HeadPose` は上下が逆で、
そのまま渡すと上を向いたら下を向く。

**左右は画面のミラー表示と揃える。** 映像を左右反転して出しているので、利用者が「映像の左に見える
目」を閉じたら 3D の頭も**見た目の左**が閉じる方が合わせやすい。解剖学的な左右（`eyeBlinkLeft` は
被写体の左目 / `wink_left` は頭の左）で素直に繋ぐと画面上では逆に出る — **実機で「右目を動かすと
CG の左が動く」と見えていたのがこれ**。yaw を鏡にしているのと同じ理由。切るなら `MIRROR_SIDES`。

**追うのをやめたら初期状態へ返す。** 表情は駆動を手放すだけで戻る（手のスライダー = 0 が効く）が、
**首は最後に追っていた向きで残る**ので正面へ返す。

**生の値をパネルに出す。** 目の `eyeBlink` L/R と、そこから作ったまばたき / ウィンク、首の角度を
トラッキング中だけ 1 行で出す。合わないときに「検出が拾えていない」のか「配分の式」なのか「重みが
削られた」のかは、**数を見ないと分けられない**。

**ARKit 52 とプリセットは 1 対 1 ではない。** 片方にしかない動きは埋めない — 近そうな項目を当てると
別の表情のときに勝手に立つ方が害が大きい。カメラで駆動されないプリセットは起動時に console へ出る。

#### なぜ「MediaPipe の値どおり」に動かないのか

**GNM の頭は部位ごとの blendshape を持たない。** アセットにあるのは公式 CVAE の
**顔全体の表情 20 本**（+ web で足した口形 5 本）で、`jawOpen` に対応する「顎だけ開く」プリセットは
無い。だから対応は「近い顔つきのプリセットへ寄せる」ことしかできず、`jawOpen` → `stretch_face` は
口を開けると**目や頬も一緒に動く**。複数の部位を同時に動かすと、プリセットが顔全体で干渉する。

読んでいる値は MediaPipe の blendshape スコアそのもので、こちらで作った値は無い。ただし
そのままでは動きが足りないので、次の 3 つを挟んでいる — **どれも足りなさの埋め合わせで、値の
差し替えではない**:

| 挟んでいるもの | 何のため |
|:--|:--|
| デッドバンド 0.08 | 無表情でも 52 カテゴリ全部に小さな値が返るので、それを捨てる |
| 追従の強さ（既定 1.8・画面で調整） | スコアははっきり作った表情でも 0.4〜0.7 しか出ない。素通しすると顔がほとんど動かない |
| 合計の上限（口 1.5 / 目 1.0） | プリセットは加算変位なので、重ねると顔が壊れる |

**部位ごとに素直に動かしたいなら、アセットに公式の 383 成分の表情基底を載せる必要がある**（今は
焼いた 20 本しか送っていない）。そうすれば MediaPipe の部位ごとの信号を部位ごとの変形へ当てられる。
今の作りは 20 本を通す近似で、**これが追従の粗さの上限**。
