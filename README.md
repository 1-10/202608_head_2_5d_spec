...
パネルに出る位置と回転は 3D ビューのワールド（GNM 空間）の値そのもの。既定は Unity 側 `MainCamera`
の写しで、そのままの姿勢なら同じ絵になる。Unity 空間は GNM 空間の X を反転した左手系なので、正面
から外れると x と yaw の符号があちらとは入れ替わる（同じ写し方の根拠は「首と視線」の角度と同じ）。
