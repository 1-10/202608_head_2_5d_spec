...
**書き出しの進行は 3D ビューの中に出す。** 段の名前と何段目かを、済んだ段・今の段・まだの段が
分かる形で下寄せに重ねる。ツールバーの隅にも出すが、そこだけだと「どこで何が起きているか」が
結び付かない — 処理の結果が出るのは 3D ビューなので。**失敗したら段を出したまま閉じる**（どこまで
進んで落ちたかが画面に残る方が原因を追える）。段の一覧は `application/exportGuest` の
