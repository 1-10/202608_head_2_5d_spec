...
動いたときに黙って嘘になる）。駆動は 3 つ — `使わない` / `手動`（あ〜お のスライダー）/ `連続再生`
（あ→い→う→え→お を順に。立ち上がり・保持・ループ・先頭から再生を触れる）。**選んだ駆動に関わる
操作しか出さない**（「使わない」ならスライダーも出ない）ので、どちらが顔を駆動しているかはパネルの形
そのもので分かる。今かかっているものは 3D ビュー下の読み出しにも出る（表情はプリセット名、口形は
`口形 あ`）。
